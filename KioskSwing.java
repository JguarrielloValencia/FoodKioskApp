...
public class KioskSwing extends JFrame {
    ...
    private final JComboBox<String> categoryFilter = new JComboBox<>(new String[] {
            "All", "Boba Drink", "Food", "Dessert", "Add-on", "Seasonal Special"
    });
    ...
    private final JButton topSellersBtn;
    ...
    public KioskSwing() {
        ...
        inventoryModel.setRows(inventory.all()); // initial
        inventoryTable.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
        inventoryTable.setFillsViewportHeight(true);
        inventoryTable.setRowHeight(26);

        // Header align + right align numeric cols
        alignInventoryColumns();

        // Top toolbar: search + category + dark mode
        ...
        // Right panel: cart + subtotal + cart controls
        ...
        topSellersBtn = new JButton("Top Sellers");
    ...
    private void onAddToCart() {
        ...
        int qty = ((Number) qtySpinner.getValue()).intValue(); // FIX: Object -> int
    ...
    private void onAdjustCart(int delta) {
        ...
        if (newQty <= 0) {
            ...
            cart.lines().put(cl.product, Integer.valueOf(newQty));
    ...
    private void onCheckout() {
        ...
        // consume stock, log order, clear cart
        Map<Product,Integer> snapshot = new LinkedHashMap<>(cart.lines());
        snapshot.forEach((p, q) -> { try { p.consume(q); } catch (Exception ignored) {} });
    ...
    private void onAdmin() {
        ...
        if (!adminLoggedIn) {
            ...
            if (pin == null || !ADMIN_PIN.equals(pin.trim())) {
                info("Access denied.");
                return;
            }
            setAdminState(true);
        ...
        try {
            ...
            inventory.restock((Integer) selected.getId(), addQty);
            inventoryModel.setRows(inventory.all());
            safeSave();
    ...
    private class CartTableModel extends AbstractTableModel {
        ...
        void setLines(Map<Product,Integer> map) {
            rows = map.entrySet().stream()
                    .map(e -> new CartLine(e.getKey(), e.getValue()))
        ...
        @Override public void setValueAt(Object aValue, int r, int c) {
            ...
            try {
                ...
                if (newQty <= 0) {
                    cart.lines().remove(rows.get(r).product);
                } else {
                    cart.lines().put(rows.get(r).product, Integer.valueOf(newQty));
    ...
    private class InventoryTableModel extends AbstractTableModel {
        ...
        private List<Product> full = new ArrayList<>();
        private List<Product> filtered = new ArrayList<>();

        void setRows(List<Product> products) {
            full = new ArrayList<>(products);
            applyFilter("", "All");
        }
